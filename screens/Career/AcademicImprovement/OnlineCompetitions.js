...
const OnlineCompetitions= (props) => {
  ...
  const closecompetitionModalWithoutSaving = () => {
    ...
    const editForm = (formIndex) => {
        ...
        setCompetitionName(formToEdit.competitionName);
  ...
  const handlecompetitionSave = async () => {
    const form = {
      competitionName, position, grade, year, description
    };
  ...
  <BackButton />
  <Text style={styles.modalTitle} numberOfLines={1} ellipsizeMode="tail"> Competition Participation </Text>
  <View style={{ width: 50 }} /> 
</View>
    ...
    {/* Check if savedForms array is empty and display message */}
     ...
     {savedForms.map((form, index) => (
      ...
      <View style={styles.textContainer}>
        ...
        <Text style={styles.buttonSubtitle}>Competition Name: {form.competitionName || '---'}</Text>
      </View>
  ...
  <View style={styles.modalView}>
    ...
    {selectedForm && ( // Ensure selectedForm is checked before rendering its details
      <ScrollView>
        ...
        <Text style={styles.fieldValue}>{selectedForm?.competitionName || '---'}</Text>
