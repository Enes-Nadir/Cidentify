...
const ClubActivities = (props) => {
  ...
  useEffect(() => {
    const loadSavedForms = async () => {
      try {
        const savedFormsString = await AsyncStorage.getItem('clubActivitiesForms');
        if (savedFormsString) {
  ...
  const handleClubSave = async () => {
    const form = { clubType, participationGradeLevel, timingOfParticipation, hoursSpentPerWeek, weeksSpentPerYear };
    if (!clubType.trim()) {
      Alert.alert('Error', 'Club Type is required');
      return;
    }
  ...
  const editForm = (formIndex) => {
    ...
    setClubType(formToEdit.clubType);
  ...
  const addNewForm = () => {
    ...
    setClubFormVisible(true); // Show the form modal
  };
      ...
      {savedForms.map((form, index) => (
        <TouchableOpacity key={index}
          ...
          <View style={styles.textContainer}>
            ...
            <Text style={styles.buttonSubtitle}>Subject/Course: {form.clubType || '---'}</Text>
          </View>
    ...
    <View style={styles.headerContainer}>
      ...
      >
        <View style={styles.modalView}>
          <TouchableOpacity style={styles.backButton} onPress={() => setClubFormVisible(false)}>
            <Ionicons name="arrow-back" size={24} color="black" />
          </TouchableOpacity>        
            <Text style={styles.modalTitle}>Club Activities Form</Text>
          ...
          <TouchableOpacity style={styles.saveButton} onPress={handleClubSave}>
        ...
        <View style={styles.modalView}>
          ...
          {selectedForm && (
            <ScrollView>
              ...
              <Text style={styles.fieldValue}>{selectedForm.clubType || '---'}</Text>
